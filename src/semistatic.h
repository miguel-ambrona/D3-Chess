...
// (after several moves) land on a2, and will take value 0 otherwise.
// The system of equations consists of a number of logical implications relating these variables.
// For instance, assuming there is a white knight on e3 and square a2 is currently empty, we will
// consider the implication:
//
//   X(e3->a2) => X(e3->b4) \/ X(e3->c3) \/ X(e3->c1) ,
//
// which represents the fact that: if the knight can go from e3 to a2, it must also be able to go
// from e3 to b4, c3 or c1 (i.e. one of the squares from which a2 can be reached).
// We say this is a "move-predecessor" implication.
//
...
//
// This way, the first implication can be made slighly more "complete" (capturing the case where a2
// is not empty) if written as:
//
//   X(e3->a2) => Clear(a2,white) /\ { X(e3->b4) \/ X(e3->c3) \/ X(e3->c1) }      (#1) .
//
...
// system as a graph where every Boolean varaible is represented by a node (there may be additional
// auxiliary nodes not corresponding to any variable). Nodes are connected (according to the system
...
//
// Solving the system can be interpreted as saturating the graph in the following sense:
//
//   1. Highlight all nodes X(s->s) for all squares 's' that are occupied in the initial position.
//      Highlight all nodes Clear(s,c) for all (s,c) if 's' does not contain a piece of color 'c'.
//
//   2. If a node V is not highlighted and ALL its MUST-predecessors are highlighted and
